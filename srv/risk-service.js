 ...
 * THIS IS THE RECOMMENEDED APPROACH FOR CUSTOMIZING FIORI ELEMENTS
 */
module.exports = cds.service.impl(async function(){
    //Define constants for the Risks and BuinsessPartners entitites from the risk-sevice.cds
    //Se ponen los nombres de las entitites
    const { Risks, BusinessPartners } = this.entities;
    /*
        Set criticallity after a READ opeation on /risks
    */
                ...
                res.map(async (risk) => {
                    const bp = await BPsrv.transaction(req).send({
                        query: SELECT.one(this.entities.BusinessPartners)
                            ...
                            //  "Customer","Supplier","AcademicTitle","Industry","GenderCodeName","Language",
                            //  "LegalForm","OrganizationFoundationDate","OrganizationLiquidationDate","SearchTerm1","SearchTerm2","AdditionalLastName",
                            //  "BirthDate","BusinessPartnerBirthDateStatus","BusinessPartnerBirthplaceName","BusinessPartnerIsBlocked","BusinessPartnerType",
                            //  "ETag","GroupBusinessPartnerName1","GroupBusinessPartnerName2","IndependentAddressID","InternationalLocationNumber3","MiddleName"
                            ]),
                        headers: {
                            apikey: process.env.apikey,
                        },
